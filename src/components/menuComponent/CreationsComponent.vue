<template>
  ...
  <div class="tw-bg-thirdly tw-flex tw-flex-col tw-h-dvh tw-justify-center tw-items-center">
    ...
    <div class="banner tw-relative tw-bg-black/25 tw-w-full tw-h-16 tw-py-2 -tw-mt-14 tw-mb-3">
      <q-avatar size="auto" color="white" :style="avatarStyle[v]" class="avatar tw-absolute" v-for="v in languagesIcons.length - 1" :key="v" >
        <q-img ratio="1.5" fit="contain" :src="'https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/' + languagesIcons[v] + '.svg'" alt="parallax1" />
      </q-avatar>
    </div>

    <div class="tw-w-full tw-flex tw-justify-evenly tw-mt-5">
      <q-input outlined v-model="inputValues.title" bg-color="white" label="Chercher un titre" @update:modelValue="SearchFilter" class="tw-w-1/5 tw-border tw-border-solid tw-border-black tw-rounded-[5px]"/>
      <q-select clearable outlined v-model="inputValues.language" bg-color="white" label="Langages & Frameworks" :options="languagesName" @update:modelValue="SearchFilter" class="tw-w-1/5 tw-border tw-border-solid tw-border-black tw-rounded-[5px]"/>
    </div>

    <div v-if="projects.length < 1" class="tw-flex tw-justify-center tw-w-full tw-my-60 tw-text-white tw-font-bold tw-text-2xl">
      <p>Aucun résultat trouvé pour cette recherche</p>
    </div>

    <div v-else class="tw-flex tw-justify-around tw-my-10 tw-w-full">
      <div v-for="project in projects" :key="project" v-tilt="tiltOptions" class="tw-relative card tw-w-1/6 tw-min-h-[27rem] tw-p-1 tw-z-10">
